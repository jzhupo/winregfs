...
int get_dword(struct hive *hdesc, int vofs, char *path, int exact)
{
  ...
  if (!v) {
	  LOG("get_dword: get_val2buf failed for %s offset %d\n", path, vofs);
	  return -1; /* well... -1 COULD BE THE STORED VALUE TOO */
  }
  dword = (int)v->data;
  FREE(v);
...
struct vk_key *add_value(struct hive *hdesc, int nkofs, char *name, int type)
{
  ...
  if (vlist_find(hdesc, nk->ofs_vallist + 0x1004, nk->no_values, name, TPF_EXACT) != -1) {
    LOG("add_value: value %s already exists\n",name);
    return NULL;
  }
...
int del_allvalues(struct hive *hdesc, int nkofs)
{
  ...
  nk = (struct nk_key *)(hdesc->buffer + nkofs);
  if (nk->id != 0x6b6e) {
    LOG("del_allvalues: Key pointer not to 'nk' node\n");
    return 1;
  }

  if (!nk->no_values) {
    LOG("del_avalues: Key at offset %x has no values\n", nkofs);
    return 1;
  }
 ...
 * return: 0 - ok, 1 fail
 */
int rdel_keys(struct hive *hdesc, char *path, int vofs)
{
  struct nk_key *key;
  int nkofs;
  struct ex_data ex;
  ...
  if (!path || !*path) {
	  LOG("rdel_keys: Null or empty path given\n");
	  return 1;
  }
			    ...
			    int curnk, char *path)
{
  ...
  if (!path && !curnk) {
	  LOG("get_class: NULL path and NULL NK given\n");
	  return NULL;
  }
  ...
  if (!clen) {
    DLOG("get_class: Key has no class data: %s\n", path);
    return NULL;
  }
